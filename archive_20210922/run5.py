...
def run(args):
    ...
    temporary_path = './tmp/' + datetime.datetime.now().strftime("%Y%m%d_%H%M%S") + '/'
    # shutil.copytree(src, dest) 
    shutil.copytree(output_path, temporary_path)
    ...
    if rank == 0:
        writer = tensorboardX.SummaryWriter(log_dir=output_path)
    ...
    # env = Env(PATH=output_path, args=args)
    # pol = Model(args.robot, env=env, ob_size=env.ob_size, ac_size=env.ac_size, im_size=env.im_size, args=args, PATH=output_path, horizon=horizon, writer=writer, max_timesteps=args.max_ts, vis=args.vis)

    env = env_builder.build_laikago_env( motor_control_mode = robot_config.MotorControlMode.POSITION, enable_rendering=enable_rendering)
    pol = Model(args.robot, env=env, ob_size=28, ac_size=12, args=args, PATH=output_path, horizon=10000, writer=writer, max_timesteps=args.max_ts, vis=args.vis)
    ...
    if args.test:
        if args.hpc:
            pol.load(home + '/hpc-home/results/quadruped/latest/' + args.folder + '/' + args.exp + '/')      
        else:
            pol.load(temporary_path)      
    ...
    # ===============================================================
    ob = env.reset()
    im = np.zeros([48,48,4]) # env.get_im()
    ep_ret = 0
    ep_len = 0
    ep_rets = []
    ep_lens = []
    ...
    pol_max_timesteps=30000000.0
    pol_timesteps_so_far = 0.0
    while True: 
        pol_timesteps_so_far += 1
        if pol_timesteps_so_far > pol_max_timesteps:
            break 
        act, vpred, _, nlogp = pol.step(ob, im, stochastic=stochastic)
        # act = (act/10) + laikago.INIT_MOTOR_ANGLES

        next_ob, rew, done, _ = env.step(act)
        next_im = np.zeros([48,48,4]) # env.get_im()
        ...
        if not args.test and ep_steps % horizon == 0:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--robot', default='quadruped', help='biped or quadruped')
...
"""
    ...
    min_eps=0.01, eps_decay=3000, min_decay=0.001, just_setup=False, just_dqn=False, old_rew=False, use_classifier=False, sim_type='pb', robot='quadruped', alg='ppo')
"""
...
def calculate_reward(self, env):
    ...
    euler_angles = pyb.getEulerFromQuaternion(orientation)
    yaw = euler_angles[2]
    yaw -= math.pi/2 
    if yaw < -math.pi:
      yaw += 2*math.pi 
    ...
    # Energy - avoid spasms
    # E = 1*(np.exp(-env.robot.GetEnergyConsumptionPerControlStep()))
  
    # Height - sigmoid, be above 0.3m
    # don't throw a tantrum, don't lie don't and flail your legs
    H = 1/(1+np.exp(-100*(z-0.3)))

    # Orientation reward from quaternions
    default_initial_orientation = pyb.getQuaternionFromEuler([math.pi / 2.0, 0, math.pi / 2.0])
    _, orientation = pyb.getBasePositionAndOrientation(uid)
    O = 3*np.exp(-np.dot(default_initial_orientation, orientation))

    # Velocity - go that way
    V = 2*(base_velocity[0]) 

    print({ "height": round(H,1), "orientation": round(O,1), "velocity": round(V,1)})   # , "symmetry": round(S,1), "periodicity": round(P,1) })
...
def calculate_reward1(self, env):
    uid = env.robot.quadruped
    pyb = env._pybullet_client
    ...
    # Height - don't throw a tantrum, don't lie don't and flail your legs
    # sigmoid, above 0.3m
    H = (2*( 1/(1+np.exp(-100*(z-0.3))) -1/2 )) 

    # Orientation - don't run sideways, maintain yaw angle; z euler angle
    O = 3*(np.exp(-7*yaw**2))
    # Orientation - roll
    O2 = (np.exp(-10*roll**2))

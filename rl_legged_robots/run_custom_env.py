...
def prep_env(args):
    ...
    for env in gym.envs.registry.all():
        if env.id == args.env:
            env_id = env.id
            env_type = env._entry_point.split(":")[0]
...
if __name__=="__main__":
    ...
    policy = build_policy(env, known.etwork)
    nenvs = 1
    ...
    if isinstance(known.learning_rate, float):
        lr = make_callback(known.learnning_rate)
    if isinstance(known.cliprange, float):
        cliprange = make_callback(known.liprange)
    ...
    nupdates = known.total_timesteps//nbatch
